  ...
  return (
    <AppShell>
      <div className="space-y-6">
        {/* Header */}
        <div className="p-6 rounded-2xl bg-gradient-to-r from-purple-950/40 via-slate-900 to-slate-900 border border-slate-800 shadow-xl">
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs text-slate-400">All data synchronized with active hiring partners</span>
          </div>
          ...
          <div className="space-y-4">
            {loops.map((loop) => (
              <div
                key={loop.id}
                className="p-6 rounded-2xl bg-slate-900/80 border border-slate-800 shadow-lg space-y-4"
              >
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-800 pb-3">
                  <div>
                    <div className="flex items-center gap-2">
                      <h3 className="text-base font-bold text-white">{loop.skill}</h3>
                      <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-purple-950 text-purple-300 border border-purple-800 font-semibold uppercase">
                        {loop.status}
                      </span>
                    </div>
                  ...
                  <div className="text-right shrink-0">
                    <span className="text-xs font-mono font-bold text-emerald-400 bg-emerald-950/80 border border-emerald-800/60 px-2.5 py-1 rounded-lg">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-xs">
                  <div className="p-3 rounded-xl bg-slate-950 border border-slate-800">
                    <span className="text-[10px] text-amber-400 uppercase font-semibold">1. Industry Demand Signal</span>
                    <p className="font-medium text-white mt-1">{loop.industryDemand}</p>
                  </div>

                  <div className="p-3 rounded-xl bg-slate-950 border border-slate-800">
                    <span className="text-[10px] text-red-400 uppercase font-semibold">2. Student Cohort Deficit</span>
                    <p className="font-medium text-white mt-1">{loop.studentProficiency}</p>
                    <p className="text-[10px] text-slate-500 mt-0.5">{loop.studentsImpacted} students affected</p>
                  </div>

                  <div className="p-3 rounded-xl bg-slate-950 border border-slate-800">
                    <span className="text-[10px] text-purple-400 uppercase font-semibold">3. Academic Intervention</span>
                    <p className="font-medium text-white mt-1">{loop.recommendedIntervention}</p>
                  </div>
                </div>

                <div className="p-3 rounded-xl bg-emerald-950/30 border border-emerald-800/40 text-xs flex items-center justify-between">
                  ...
                  <Link
                    href="/admin/training"
                    className="text-emerald-400 hover:underline font-semibold shrink-0 ml-4"
                  >
